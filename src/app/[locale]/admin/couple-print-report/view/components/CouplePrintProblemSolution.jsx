 ...
 */
/** Align with couple print report: CoreSuggestion / MingJu use #A47584, #8B7355; female/male from P2 bars */
const COUPLE_ACCENT = "#A47584";
const COUPLE_BAR_BG = "#8B7355";
const FEMALE_ACCENT = "#AA7C8C";
const MALE_ACCENT = "#597E9C";
const SECTION_TITLE_COLOR = "#333";
const CARD_HEADER_BG = "#A47584";
...
function categorizeProblem(problem) {
	...
	if (/異地|長距離|工作|家庭|父母|環境|壓力/.test(p))
		return { categoryKey: "specialSituation", label: "特殊情境類" };
...
export default function CouplePrintProblemSolution({
	...
	const hasSubsections =
		isEmotionCooling &&
		subsections &&
		(subsections?.chartDiagnosis ||
			subsections?.emergencyFengShui ||
			subsections?.restartChemistry);

	const chartDiagnosis = hasSubsections ? subsections.chartDiagnosis : null;
	const emergencyFengShui = hasSubsections
		? subsections.emergencyFengShui
		: null;
	const restartChemistry = hasSubsections
		? subsections.restartChemistry
		: null;
		...
		<div className="mx-auto bg-white page-break" style={pageStyle}>
							...
							>
								...
								<span
									style={{
										padding: "3px 8px",
										borderRadius: "9999px",
										border: `2px solid ${FEMALE_ACCENT}`,
										background: "#fff",
										fontSize: "10px",
										color: FEMALE_ACCENT,
									}}
			...
			{emergencyFengShui?.recommendations?.length > 0 && (
				<>
					...
					>
						{emergencyFengShui.recommendations.map(
							(item, index) => (
								<div
									key={index}
									style={{
