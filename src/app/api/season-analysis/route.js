...
4. **严格格式标准**：每个季节必须遵循以下结构
   ...
   - 主要说明：**1个段落，80-120字（约4行）**，简要说明该季节如何影响${concern}；不要在本段内写「本月剩余时间实用行动计划」或任何列表，行动计划放在「具体建议：」下面
   - 具体建议部分：以"**具体建议：**"开头，列举3条建议（可含「本月剩余时间」行动），每条以"- "开头
...
2. 主要说明：**一段话，80-120字（约4行）**，说明该季节对${concern}的影响；禁止在本段写「本月剩余时间实用行动计划」或任何 bullet，本月行动写在「具体建议」里
3. 具体建议部分：以"**具体建议：**"开头，列举3条简短建议（每条15-20字；「本月剩余时间」行动写在这里）
4. 简单禁忌部分：以"**简单禁忌：**"开头，列举1条禁忌

示例格式（每个季节必须包含这四部分；主要说明80-120字约4行，不写行动计划）：
【当前季节】秋季（申酉戌月，金旺）
秋季是金气最强的时候，就像秋天收获一样。金代表理性和判断，对${concern}而言此时适合做重要决定、收敛与规划。气候转干燥，需注意保养，避免过度消耗。
...
**特别要求：**
- **主要说明 = 一段话，80-120字（约4行）**，说明季节特性及对${concern}的影响；禁止在主要说明内写「本月剩余时间实用行动计划」或任何列表，本月行动写在「**具体建议：**」下面
- 当前季节${currentSeasonName}的「本月剩余时间」行动提示写在「具体建议」中
- 每个分析用白话说明"为什么"会这样影响，内容充实但控制在约4行内
- 使用现代人容易理解的表达方式
- 重点突出季节变化对日常生活的实际影响
- **主要说明约80-120字（不超过约4行）；过长内容移到「具体建议」**
	...
	try {
		...
		seasons.forEach((season, seasonIdx) => {
			...
			if (seasonContent && seasonContent.length > 20) {
				...
				// Cap description to ~4 lines (120 chars) for season page display
				const MAX_DESCRIPTION_CHARS = 120;
				if (seasonContent.length > MAX_DESCRIPTION_CHARS) {
					const head = seasonContent.substring(0, MAX_DESCRIPTION_CHARS);
					const lastPeriod = head.lastIndexOf("。");
					seasonContent =
						lastPeriod >= 0
							? seasonContent.substring(0, lastPeriod + 1)
							: head;
				}
