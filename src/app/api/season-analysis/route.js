...
export async function POST(req) {
	try {
		...
		const prompt = `你是資深八字命理分析師。請為用戶生成詳細的四季運勢分析，必須使用繁體中文和易懂的白話文：
...
**重要要求：**
1. **必須使用繁體中文**，禁止任何簡體字
2. **使用白話文解釋**，避免過於艱深的專業術語，讓一般人都能理解
3. **季節順序**：按當前季節開始，依次分析四季（${relevantSeasons.join(" → ")}）
4. **內容深度**：**每季節120-150字的深度分析，用簡單易懂的語言解釋五行理論**
5. **格式標準**：#### **季節名【狀態標籤】**（月份，五行特性）：詳細分析內容
6. **⚠️ 關注領域限制**：用戶關注「${concern}」，**絕對禁止提及其他領域的建議**
   - 如果是「事業」：只談工作、職場、商業、合作、決策等
   - 如果是「感情」：只談戀愛、婚姻、人際關係等  
   - 如果是「財運」：只談投資、理財、收入、支出等
   - 如果是「健康」：只談身體、養生、保健等
   - **嚴禁跨領域建議**：事業報告不可包含健康建議，健康報告不可包含財運建議等
6. **生活化解釋**：用日常生活的例子來解釋五行影響，而不是艱深的命理術語

**分析重點（每季節必須包含，但用白話文表達）：**
- **季節特性**：用簡單語言解釋該季節五行的基本特點（如：春天木旺就像樹木生長，夏天火旺就像太陽很熱）
- **對身體影響**：用白話解釋五行對身體的影響機制（如：金克木就是說秋天的涼意會影響肝臟，因為中醫認為肝屬木）
- **對${concern}的影響**：用生活化的語言解釋對關注領域的具體影響
- **時間重點**：提及重要時期但用白話說明（如：「白露之後」改為「9月中旬開始」）
- **具體建議**：提供3-4個簡單易做的生活建議
- **簡單禁忌**：基於五行原理但用白話表達的注意事項

**語言要求：**
- **避免過多專業術語**：如「七殺透干、偏印、財生殺攻身」等艱深詞彙
- **用白話解釋原理**：如「金克木」在「${concern}」方面的具體影響
- **生活化表達**：如「金氣當令」改為「秋天金的能量最強」
- **簡化時辰**：避免用「申時酉時」，改用「下午3-5點」等現代表達
- **實用建議**：提供與「${concern}」相關的容易執行方法
...
**特別要求：**
- 當前季節${currentSeasonName}需要額外詳細，包含"本月剩餘時間的實用行動計劃"
- 每個分析都要用白話解釋"為什麼"會這樣影響，但保持專業性
- 使用現代人容易理解的表達方式
- 重點突出季節變化對日常生活的實際影響
- **確保內容豐富但易懂，每季節控制在120-150字範圍內**
- **絕對禁止使用簡體字，全文必須繁體中文**

**五行季節對應關係（用白話解釋）**：
- 春季（2-4月）：木的能量最強，就像植物生長，對肝臟最好，但可能影響脾胃
- 夏季（5-7月）：火的能量最強，就像夏日炎熱，對心臟有影響，容易上火
- 秋季（8-10月）：金的能量最強，就像秋風涼爽，對肺部好，但可能傷肝
- 冬季（11-1月）：水的能量最強，就像冬季寒冷，對腎臟重要，要注意保暖

**最終檢查要求：**
- 再次確認用戶關注「${concern}」，所有建議必須100%聚焦於此領域
- 如果發現任何跨領域內容，立即修改為${concern}相關建議
- 每個季節分析都要明確說明對「${concern}」的具體影響和行動建議

請用白話文但保持專業內容，生成完整的四季分析，讓一般人都能看懂並實際應用。`;
