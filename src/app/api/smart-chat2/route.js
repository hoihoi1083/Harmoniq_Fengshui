...
function parseCouplesBirthdays(message) {
	...
	const couplePatterns = [
		// 男XXX，女XXX (direct gender specification)
		{
			pattern:
				/男方?\s*(\d{4}[年\-\/]\d{1,2}[月\-\/]\d{1,2}[日]?)[，,\s]*女方?\s*(\d{4}[年\-\/]\d{1,2}[月\-\/]\d{1,2}[日]?)/,
			userGender: "male",
			partnerGender: "female",
		},
		// 女XXX，男XXX (direct gender specification, reversed)
		{
			pattern:
				/女方?\s*(\d{4}[年\-\/]\d{1,2}[月\-\/]\d{1,2}[日]?)[，,\s]*男方?\s*(\d{4}[年\-\/]\d{1,2}[月\-\/]\d{1,2}[日]?)/,
			userGender: "female",
			partnerGender: "male",
		},
...
小鈴會先給你一個簡單的命理分析，如果你覺得有幫助，還可以做更詳細的完整八字報告哦～🔮💕`;
	...
	async generateDefaultFlow(analysis, originalMessage) {
		try {
			// 🤖 總是生成新的詳細AI回應，提供更好的用戶體驗
			const specificProblem = analysis.specificProblem || originalMessage;
			let topic = analysis.detectedTopic || "運勢";
			// Map 工作 to 事業 for display
			if (topic === "工作") topic = "事業";
...
重要指示：1. ${languageInstruction} 2. 在回應中所有日期和月份都必須使用新歷（西曆/公曆），例如1月、2月、3月等，不要使用農歷 3. 不要在回應中包含字數統計標記 4. 如果需要提及時間，請確保是當前的2025年，避免提及2024年或過去的時間 5. 只在真正需要時間背景的情況下才提及日期，不要強制在每個回應中都包含 6. 2025年是蛇年，不要誤稱為馬年或其他生肖年 7. 術語規定：在所有回應中必須使用「事業」而非「工作」來描述職場相關話題，例如「事業分析」「事業運勢」「事業方面」。

我們提供的服務領域：
- 感情：愛情、戀愛、桃花、分手、復合、婚姻、單身（僅限浪漫愛情關係）
- 財運：賺錢、投資、理財、偏財運、正財運、個人財富  
- 事業：升職、跳槽、職場運勢、事業發展、工作機會、生意經營、創業、公司營運、商業決策
- 健康：身體健康、疾病、養生、健康運勢
...

{
	...
	"detectedTopic": "感情|財運|事業|健康|命理|其他",
...
重要分類規則：
1. 生意經營、創業、公司營運、商業決策 → 歸類為「事業」
2. 個人投資、理財、財富增長 → 歸類為「財運」 
3. 八字分析、流年運勢、生肖運勢（如馬年、蛇年影響）、命盤、紫微斗數 → 歸類為「命理」
4. 親情（家人、父母、子女）、友情（朋友）、一般人際關係 → 必須歸類為「命理」
5. 感情類別僅限浪漫愛情關係（戀愛、分手、復合、婚姻、桃花、單身）→ 歸類為「感情」
6. 如果用戶輸入很簡單（如只是"感情"、"財運"、"命理"），specificProblem應該保持簡潔，如"一般感情諮詢"或"一般命理諮詢"
7. 如果問題不在我們服務範圍內（如天氣、科技、日常閒聊等），請設定 isWithinScope 為 false，並在 aiResponse 中提供禮貌友善的回應，自然地確認用戶的話題
8. 對於數字或選擇類輸入（如"1"、"2"），通常表示用戶在回應選項，設為不在範圍內
9. aiResponse 應該禮貌回應用戶，不要直接說"不在服務範圍"，而是自然地承接話題
10. 在 serviceRecommendation 中智能引導到相關的風水服務
...
我們提供的服務領域：
- 感情：愛情、戀愛、桃花、分手、復合、婚姻、單身、約會、結婚（僅限浪漫愛情關係）
...
2. 所有時間相關回應必須基於當前日期${currentDateStr}
3. 不要提及2024年或過去的時間
4. 不要在回應中包含字數統計標記
5. 保持小鈴親切可愛的語氣風格`,
					},
...
export async function POST(request) {
	...
	try {
		...
		userEmail =
			requestData.userEmail || userEmailFromSession || "anonymous";
		userId =
			requestData.userId ||
		...
		let region; // locale already declared at top level
		if (
			clientRegion &&
		...
		if (userBirthday && gender && !message?.trim()) {
			...
			if (!userIntent) {
				...
				if (reportType && reportType.includes("感情"))
					...
					mappedConcern = "財運";
				else if (reportType && reportType.includes("事業"))
					mappedConcern = "事業";
		...
		if (specificServiceRequest) {
			...
			if (existingSavedBirthday?.userBirthday) {
				...
				// Map 工作 to 事業 for display
				let topicText = specificServiceRequest.detectedTopic || "命理";
				if (topicText === "工作") topicText = "事業";
...
請回覆「1」或「2」～`;
		...
		if (shouldProcessCouple) {
			...
			try {
				...
				// Map 工作 to 事業 for display
				const displayTopic =
					topicAndBirthdayData.topic === "工作"
						? "事業"
						: topicAndBirthdayData.topic;
				// 備用回應
				response = `🎉 太好了！你想了解${displayTopic}，生日是 ${topicAndBirthdayData.birthday.original}！

讓小鈴為你做詳細的${displayTopic}分析～
...
小鈴會先給你一個簡單的分析，然後你可以選擇是否需要更詳細的報告！✨`;
			...
			try {
				...
				} else if (userIntent.primaryConcern === "事業") {
					response =
						await EnhancedInitialAnalysis.generateWorkAnalysis(
							new Date(standardDate),
...
「我1995年3月15日，她1996年8月20日」
「我的生日是1996年8月20日，他的生日是1995年3月15日」
「男1999/3/4，女2000/3/4」`
...
請直接給出解釋，不要說"我來解釋"之類的開場白。`;
		...
		try {
			...
			// 🔧 FIX: Removed duplicate save - now handled in finally block
		} catch (dbError) {
		...
		if ((message || response) && sessionId && userId) {
			try {
				...
				console.log(
					"✅ Conversation saved successfully in finally block"
				);
			} catch (saveError) {
				console.error(
					"🚨 CRITICAL: Failed to save conversation in finally block:",
					saveError
				);
				...
				console.error("   Response:", response ? "present" : "missing");
				// TODO: Implement retry mechanism or alert admin
			}
		} else {
			console.log(
				"⚠️ Finally block - Skipping save (missing required data):",
				{
					hasMessage: !!message,
					hasResponse: !!response,
					hasSessionId: !!sessionId,
