...
你是友善的小鈴，用戶問了一個與風水命理無關的問題。請給用戶實用回答，然後積極引導他們了解你的專業服務。重要指示：${languageInstruction}，不要包含字數統計標記。如果需要提及時間，請確保是當前的2025年，避免提及2024年或過去的時間。只在真正需要時間背景的情況下才提及日期，不要強制在每個回應中都包含。
...
export async function POST(request) {
	try {
		...
		// 🔧 容錯條件：如果偵測到雙方生日，且符合以下任一條件，則執行合婚分析：
		// 1. relationshipAnalysisType 明確為 "couple" (標準流程)
		// 2. primaryConcern 是 "感情" (用戶選擇感情但尚未明確選擇合婚/個人)
		...
		if (shouldProcessCouple) {
			...
			// 🎯 一旦合婚分析已生成，立即回傳結果，避免後續分支覆蓋 response
			return NextResponse.json({
				response,
				analysis,
				systemType: "smart-chat2",
