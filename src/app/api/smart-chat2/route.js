...
import { getTranslation } from "@/lib/chatTranslations"; // 🌐 新增：翻譯支援
...
function formatCoupleAnalysisResponse(analysisResult, locale = "zh-TW") {
	...
	if (analysisResult.reportRecommendation) {
		...
		const header = getTranslation(locale, "reportPromotionFooter.header");
		const coupleText =
			locale === "zh-CN"
				? `\n根据你们的状况，风铃为你们推荐：\n\n`
				: `\n根據你們的狀況，風鈴為你們推薦：\n\n`;
		formattedResponse += header + coupleText;
...
我會幫你看正財偏財、投資理財和招財開運方法喔～✨`,
	...
	// 🎯 測算帶有分析額度信息的生日收集模板
	async generateBirthdayTemplate(
		userEmail,
		userId,
		topicText = "運勢",
		locale = "zh-TW"
		...
		try {
			...
			rateLimitInfo = getTranslation(
				locale,
				"dailyAnalysisQuota",
				dailyLimit,
				remainingAnalyses,
				locale === "zh-CN" ? "专业分析" : "專業分析"
			);
		} catch (error) {
		...
		const askBirthday =
			locale === "zh-CN"
				? `\n\n告诉风铃你的生日，我可以帮你看看${topicText}方面的运势哦！\n\n`
				: `\n\n告訴風鈴你的生日，我可以幫你看看${topicText}方面的運勢哦！\n\n`;

		const birthdayExamples = getTranslation(
			locale,
			"birthdayFormatExamples"
		);

		return askBirthday + birthdayExamples + rateLimitInfo;
	}
...
- 不要列出所有5個服務選項`;
	...
	async generateCareerFlow(analysis, originalMessage) {
		try {
			...
			// 🌐 Character name based on locale
			const fenglingName = this.locale === "zh-CN" ? "风铃" : "風鈴";
...
請以${fenglingName}的身份，針對這個具體的工作問題，提供自然對話式的回應。
...
- 提供多個層面的實用建議，自然融入對話中
- 可以分享職場智慧、人生感悟
- 語氣要溫暖親切且有深度，像${fenglingName}在面對面深談
- 適當使用emoji增加親切感
- 最後自然地邀請分享生日做進一步分析
...
請以${fenglingName}的身份，針對這個具體的財運問題，提供自然對話式的回應。
...
- 提供多個層面的理財建議，自然融入對話中
- 可以分享投資智慧、財務管理經驗
- 語氣要溫暖親切且專業，像${fenglingName}在面對面深談理財
- 適當使用emoji增加親切感
- 最後自然地邀請分享生日做進一步分析
...
重點關注: 深度財務分析、理財智慧、投資策略、財務心理學`
			...
			return (
				aiResponse +
				`\n\n` +
				getTranslation(this.locale, "birthdayInvitationFull", "財運")
			);
		} catch (error) {
			...
			const fallbackMessage =
				this.locale === "zh-CN"
					? `哇～你想了解财运呀！风铃最喜欢帮人解决问题啦！✨\n\n每个人的运势都不一样呢，就像每个人的生日不一样一样！\n\n`
					: `哇～你想了解財運呀！風鈴最喜歡幫人解決問題啦！✨\n\n每個人的運勢都不一樣呢，就像每個人的生日不一樣一樣！\n\n`;
...
請以${fenglingName}的身份，針對這個具體的健康問題，提供自然對話式的回應。
...
- 提供多個層面的健康建議，自然融入對話中
- 可以分享養生智慧、健康管理經驗
- 語氣要溫暖關懷且有深度，像${fenglingName}在面對面深談健康
- 適當使用emoji增加關懷感
- 最後自然地邀請分享生日做進一步分析
...
重點關注: 深度健康關懷、養生智慧、生活品質提升、身心平衡`
			...
			const fallbackMessage =
				this.locale === "zh-CN"
					? `哇～你想了解健康呀！风铃最喜欢帮人解决问题啦！✨\n\n每个人的运势都不一样呢，就像每个人的生日不一样一样！\n\n`
					: `哇～你想了解健康呀！風鈴最喜歡幫人解決問題啦！✨\n\n每個人的運勢都不一樣呢，就像每個人的生日不一樣一樣！\n\n`;
...
請以${fenglingName}的身份，針對這個具體的命理問題，提供自然對話式的回應。
...
- 提供多個層面的人生建議，自然融入對話中
- 可以分享人生智慧、命理經驗
- 語氣要溫暖有智慧感且深度，像${fenglingName}在面對面深談人生
- 適當使用emoji增加溫暖感
- 最後自然地邀請分享生日做進一步分析
...
請以${fenglingName}的身份，針對這個具體的${topic}問題，提供自然對話式的回應。

回應風格要求:
- 用自然對話的方式回應，就像朋友聊天一樣
- 先表達理解關心，然後自然地分享一些建議
- 語氣要溫暖親切，像${fenglingName}在面對面聊天
- 避免列點式、條列式的機械化回應
- 建議要融入在對話中，不要刻意分段編號
- 適當使用emoji，但不要過多
- 最後自然地邀請分享生日做進一步分析
...
要像真人對話: 溫暖、自然、有同理心的朋友建議`
			...
			return (
				aiResponse +
				`\n\n` +
				getTranslation(this.locale, "birthdayInvitationFull", topic)
			);
		} catch (error) {
			console.error("AI測算默認回應失敗:", error);
			// 備用模板回應
			const fallbackTopic = analysis.detectedTopic || "運勢";
			const fallbackMessage =
				this.locale === "zh-CN"
					? `哇～你想了解${fallbackTopic}呀！风铃最喜欢帮人解决问题啦！✨\n\n每个人的运势都不一样呢，就像每个人的生日不一样一样！\n\n`
					: `哇～你想了解${fallbackTopic}呀！風鈴最喜歡幫人解決問題啦！✨\n\n每個人的運勢都不一樣呢，就像每個人的生日不一樣一樣！\n\n`;
...
你想了解哪一種？？？？`;
	...
	generateGreetingResponse() {
		return {
			...
			confidence: 0.95,
			aiResponse: getTranslation(this.locale, "initialGreeting"),
		...
		try {
			...
			const response = await this.callDeepSeekAPI(
				[
					{
						role: "system",
						content: `你是專業且親切的${fenglingName}，請根據用戶的具體問題提供相關的風水建議。

當前日期：${currentDateStr}（請在回應中使用這個準確的日期作為參考，不要提及過時的年份如2024年等）

重要指示：
1. ${languageInstruction}
2. 在回應中提到自己的名字時，必須使用「${fenglingName}」
3. 所有日期和月份都必須使用新歷（西曆/公曆），不要使用農歷。例如：1月、2月、3月等，避免使用農曆1月、農曆8月等表達方式
4. 🚫 嚴格禁止任何農曆用詞：農曆九月、農曆十月、農曆初七、農曆十五等
5. ✅ 正確用法：9月出生、10月出生、每月7日、15日等
6. 不要在回應中包含字數統計如（72字）等標記
7. 保持${fenglingName}親切可愛的語氣風格
8. 請確保所有時間相關的回應都基於當前日期${currentDateStr}，避免提及2024年或過去的時間。`,
					},
					{
						role: "user",
						content: prompt,
			...
			responseText = cleanLunarCalendarTerms(responseText);

			// 🔄 Convert to Simplified Chinese if locale is zh-CN
			if (this.locale === "zh-CN") {
				responseText = convertToSimplifiedChinese(responseText);
			}
...
根據你的狀態，風鈴會為你量身打造最適合的感情指導～💕`;
		...
		if (analysis.aiResponse) {
			...
			return getTranslation(
				this.locale,
				"relationshipAnalysisOptions.withAI",
				analysis.aiResponse
			);
		}
		...
		return getTranslation(
			this.locale,
			"relationshipAnalysisOptions.general"
		);
	}
	...
	async generateOutOfScopeResponse(
		...
		let response = getTranslation(
			this.locale,
			"smartChat2.thankYouResponse"
		);

		response += getTranslation(this.locale, "smartChat2.outOfScopeNote");
...
請以${fenglingName}的身份，按照以下格式測算詳細的八字分析報告：

🔮 ${fenglingName}看了你的八字，發現你有很特別的${displayTopic}潛質呢！💼
...
[五行]命寶寶記得多用「[五行相生原理]」原理，[具體建議]能讓你[效果]！✨ 有問題隨時喚醒${fenglingName}喔～
...
要求：
1. 保持${fenglingName}可愛親切的語氣，使用表情符號和可愛語助詞
2. 提供具體實用的建議，避免空泛描述
...
8. 特別注意：用戶的生日是西曆日期，不是農曆，請據此分析命理特質`;
		...
		try {
			...
			const aiResponse = await classifier.callDeepSeekAPI([
				{
					role: "system",
					content: `你是專業的風水命理顧問${fenglingName}，擅長八字分析和運勢預測。請按照指定格式測算詳細且實用的分析報告。

⚠️ 重要指示：
1. ${languageInstruction}
2. 在回應中提到自己的名字時，必須使用「${fenglingName}」
3. 所有日期和月份都必須嚴格使用西曆（公曆），如：1月、2月、3月等
4. 🚫 絕對禁止使用農曆、陰曆用詞，如「農曆十月」「陰曆八月」等
5. ✅ 正確示例：「10月出生者」「十月出生」「10月份」
6. ❌ 錯誤示例：「農曆十月出生者」「陰曆八月」「農曆四月」
7. 當前是2025年10月19日，請確保分析基於2025年，不可提及2024年
8. 用戶提供的生日是西曆日期，請據此進行命理分析
9. 不要在回應中包含字數統計標記
10. 保持${fenglingName}專業且親切的語氣
11. 如果需要提及月份，直接說「10月」「八月」等，不要加「農曆」「陰曆」前綴`,
				},
				{
					role: "user",
			...
			if (
				...
				let responseText = cleanLunarCalendarTerms(
				...
				let responseText = cleanLunarCalendarTerms(aiResponse);
	...
	try {
		...
		if (specificServiceRequest) {
			...
			try {
				...
				// 在原回應後添加額度信息
				const rateLimitInfo = getTranslation(
					locale,
					"dailyAnalysisQuota",
					dailyLimit,
					remainingAnalyses,
				...
				return NextResponse.json(
					{
						response: getTranslation(
							locale,
							"smartChat2.conversationRequired"
						),
						systemType: "smart-chat2",
					},
					{ status: 400 }
			...
			if (reportType === "detailed_concern") {
				...
				if (
					...
					try {
						...
						response = getTranslation(
							locale,
							"smartChat2.reportGenerationError"
						);
						reportUrl = null;
					}
...
正在準備你的專屬報告...`;
			...
			if (userIntent.primaryConcern === "感情") {
				if (choice === "1") {
					...
					response = getTranslation(
						locale,
						"individualAnalysisBirthdayRequest"
					);
					userIntent.relationshipAnalysisType = "individual";
...
請填寫個人資料，我們開始製作你的專屬佈局方案～`;
				...
				// 🔄 Convert Traditional Chinese to Simplified for zh-CN locale
				if (
					locale === "zh-CN" &&
					response &&
					typeof response === "string"
				) {
					response = convertToSimplifiedChinese(response);
				}
			...
			try {
				...
				if (
					...
					// 🌐 Character name based on locale
					const fenglingName = locale === "zh-CN" ? "风铃" : "風鈴";
...
請直接給出解釋，不要說"我來解釋"之類的開場白。`;
					...
					response = await classifier.callDeepSeekAPI([
						{
							role: "system",
							content: `你是專業的風水命理顧問${fenglingName}，擅長用簡潔易懂的方式解釋傳統概念。

⚠️ 重要指示：
1. ${languageInstruction}
2. 在回應中提到自己的名字時，必須使用「${fenglingName}」
3. 所有日期和月份都必須嚴格使用西曆（公曆），如1月、2月、3月等
4. 🚫 絕對禁止使用農曆、陰曆用詞，如「農曆十月」「陰曆八月」等
5. ✅ 正確示例：「10月出生者」「十月份」「8月生人」
6. ❌ 錯誤示例：「農曆十月出生者」「陰曆八月」
7. 當前是2025年10月19日，請確保分析基於2025年，不可提及2024年
8. 用戶提供的生日是西曆日期，請據此分析
9. 不要在回應中包含字數統計標記
10. 保持專業且親切的語氣`,
						},
						{
			...
			if (
				...
				if (previousBaziMessage) {
					...
					if (
						locale === "zh-CN" &&
						detailedAnalysis &&
						typeof detailedAnalysis === "string"
					) {
						detailedAnalysis =
							convertToSimplifiedChinese(detailedAnalysis);
					}
					...
					// 找不到之前的八字數據，重新請求
					const needsBirthdayMsg = getTranslation(
						locale,
						"smartChat2.needsBirthdayForAnalysis"
					);
					response = `${needsBirthdayMsg}${topicAnalysis.detectedTopic}呢！
...
請以${fenglingName}的身份，用可愛親切的語氣為用戶分析八字。八字資料：
- 八字：${enhancedResult.baziData?.baziString || ""}
- 主要分析領域：${enhancedResult.specificProblem || "八字分析"}

請按照以下格式生成回應：

🔮 ${fenglingName}看了你的八字，發現你有很特別的${enhancedResult.detectedTopic || "運勢"}潛質呢！✨
...
💖 哈囉親愛的[五行]命寶寶！${fenglingName}來幫你分析${enhancedResult.detectedTopic || "運勢"}啦～根據你的五行特質，[年份]年會是[運勢特點]的一年呢！(´▽\`ʃ♡ƪ)
...
[五行]命寶寶記得要多用[幸運顏色]系物品增強運氣呢～${fenglingName}祝你[祝福語]！٩(◕‿◕)۶
						...
						try {
							const aiResponse = await classifier.callDeepSeekAPI(
								[
									{
										role: "system",
										content: `你是專業的風水命理顧問${fenglingName}，擅長八字分析和運勢預測。
...
請直接給出解釋，不要說"我來解釋"之類的開場白。`;
					...
					if (
						response.includes("風鈴會先給你一個簡單的分析") ||
						response.includes("风铃会先给你一个简单的分析")
					) {
						try {
							const userStats =
								await DailyAnalysisRateLimit.getUserStats(
		...
		// 🔄 Convert Traditional Chinese to Simplified for zh-CN locale
		if (locale === "zh-CN" && response && typeof response === "string") {
			response = convertToSimplifiedChinese(response);
		}
		...
		return NextResponse.json(
			{
				response: getTranslation(locale, "smartChat2.systemError"),
				error: "系統錯誤",
				systemType: "smart-chat2",
			},
			{ status: 500 }
