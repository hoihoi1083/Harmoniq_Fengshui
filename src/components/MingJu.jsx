...
// Basic format gate to keep 日主特性 responses in 1️⃣/• bullet form
function isDayMasterBulletFormat(text) {
	...
	return headings && bulletCount >= 10 && lengthOk;
}
			...
			if (response.ok) {
				...
				if (tab === "日主特性") {
					...
					if (data.content && isDayMasterBulletFormat(data.content)) {
						...
						console.log(
							"AI content failed bullet validation for 日主特性",
							{
...
針對${currentYearInfo.year}年的流年特點，您需要注重個人成長與人際關係的平衡發展。`;
	...
	void concernSpecificContent; // legacy narrative retained for reference

	// Build a compact bullet template as fallback to avoid段落式輸出
	const traitMap = {
		金: "果決精準，重視效率",
		木: "成長柔韌，善於協調",
		火: "熱情外放，推動力強",
	...
	const dmCore = traitMap[baziInfo.dayMaster.slice(0, 1)] || "平衡務實";
	const strengthAdv =
...
1️⃣ 日主特質
	• ${baziInfo.dayMaster}日主${baziInfo.strength}，${baziInfo.element}命，例：注重規劃/紀律
	• ${dmCore}，${lifeStage}期保持平衡，例：帶隊協作或專精深耕

2️⃣ 優勢分析
	• 優勢1：${strengthAdv}，例：主導專案/穩健布局
	• 優勢2：${concernAdvice[concern] || "定位清晰，可持續累積"}，例：善用本業累積資產

3️⃣ 劣勢與挑戰
	• 劣勢1：${strengthWeak}，例：決策前需二次評估
	• 劣勢2：${stageWeak}，例：分工或尋找顧問支援

4️⃣ 調候與策略
	• 調候需求：${strategyNeed}
	• 實施方向：${strategyDo}，例：每週固定複盤/拓客

5️⃣ 針對${concern}的建議
	• 短期策略：${shortTerm}，例：3-6月內完成1筆升薪或副業收入
	• 長期規劃：${longTerm}，例：定投指數/分批置產/打造專業品牌`;
}
...
⚠️⚠️⚠️ 【絕對強制格式 - 必須100%遵守，違反格式直接返回錯誤】⚠️⚠️⚠️

您的回應必須完全按照以下格式。不能有任何例外，不能寫成段落形式。

【加值要求】
每個要點須包含具體例子（行業/情境/資產類型/時間節點/操作步驟），字數18-30字，避免空泛形容。
...
4️⃣ 調候與策略
	• 調候需求：[18-30字，含例子]
	• 實施方向：[18-30字，含具體步驟或時程]

5️⃣ 針對${concern}的建議
	• 短期策略：[18-30字，含3-6月內的可執行動作]
	• 長期規劃：[18-30字，含1年以上的方向/配置示例]

【驗收標準】
✅ 必須有5個一級標題（用1️⃣2️⃣3️⃣4️⃣5️⃣）
✅ 每個標題下必須有2個要點（用•開頭）
✅ 每個要點18-30字，不能超過35字，且包含具體例子
✅ 不能有任何"您的命局..."開頭的長句子
✅ 總字數220-320字
...
如果您無法完全遵守此格式，請立即停止並返回："格式錯誤，無法生成"

【示例（必須完全複製此風格）】
【財運分析】

1️⃣ 日主特質
   • 庚金自坐祿神，日支申金旺相，命局金氣強旺
   • 如鋼鐵般堅毅，追求效率，決策果斷迅速

2️⃣ 優勢分析
   • 優勢1：專業能力強，通過技能與權威獲取財富穩定
   • 優勢2：執行力強，能在壓力下激發潛能創造成果

3️⃣ 劣勢與挑戰
   • 劣勢1：比肩坐印引動同輩競爭，易有合作財務糾紛
   • 劣勢2：火旺官殺年份壓力大，易衝動做出決策

4️⃣ 調候與策略
   • 調候需求：需水來淬火，增加流動性與柔軟度
   • 實施方向：學習、進修、拓展新領域知識技能

5️⃣ 針對財運的建議
   • 短期策略：接下來2-3月專注本業表現，爭取晉升機會
   • 長期規劃：建立多元收入來源，重視實體資產積累

【重要警告】
您的回應必須100%符合上述格式。
如果任何部分不符合格式，用戶將看到"格式錯誤"。
不允許有任何變通或例外。
${languageInstruction}`;
	} else if (tab === "middle") {
		if (normalizeConcern(concern) === "財運") {
...
export function MingJu({ userInfo, currentYear, isPrintMode = false }) {
	...
	// Print-friendly content rendering
						...
						>
							...
							{(() => {
								...
								const parseSections = (content) => {
									...
									const sections =
										sectionsWithContent.slice(1);

									return sections;
								};

								const sections = parseSections(leftContent);
