...
OUTPUT_BASE_DIR = "/Users/michaelng/Desktop/marketing/药珀手串_ad_material_20260507"
...
# --- Marketing Texts ---
marketing_texts_tc = {
    "amber_cover.png": ("天然藥珀手串", "沉穩安神·古樸雅致"),
    "amber_01.png": ("溫潤質感", "靜心凝神"),
    "amber_02.png": ("古樸紋理", "歲月沉澱"),
    "amber_03.png": ("禪意生活", "尋回內心"),
    "amber_04.png": ("低調奢華", "品味非凡"),
    "amber_05.png": ("藥香暗蘊", "寧靜致遠"),
    "amber_06.png": ("日常穿搭", "彰顯氣質"),
    "amber_07.png": ("光影交錯", "溫暖如初"),
    "amber_08.png": ("靜享時光", "沉澱心靈"),
    "amber_09.png": ("傳統美學", "現代演繹"),
    "amber_10.png": ("質感細膩", "觸手生溫"),
    "amber_11.png": ("護身祈福", "平安喜樂"),
}

marketing_texts_sc = {
    "amber_cover.png": ("天然药珀手串", "沉稳安神·古朴雅致"),
    "amber_01.png": ("温润质感", "静心凝神"),
    "amber_02.png": ("古朴纹理", "岁月沉淀"),
    "amber_03.png": ("禅意生活", "寻回内心"),
    "amber_04.png": ("低调奢华", "品味非凡"),
    "amber_05.png": ("药香暗蕴", "宁静致远"),
    "amber_06.png": ("日常穿搭", "彰显气质"),
    "amber_07.png": ("光影交错", "温暖如初"),
    "amber_08.png": ("静享时光", "沉淀心灵"),
    "amber_09.png": ("传统美学", "现代演绎"),
    "amber_10.png": ("质感细腻", "触手生温"),
    "amber_11.png": ("护身祈福", "平安喜乐"),
}
...
def add_border_and_shadow(img, border_size=4, shadow_size=25, shadow_opacity=80):
    # Use a warm, aged sepia/brown border
    bordered = ImageOps.expand(img, border=border_size, fill='#8b5a2b') 
...
def process_promo():
    print("Generating gifted report promo image (Ancient Scroll Style)...")
    
    # 1. Background - Ancient Scroll / Parchment Style
    bg_path = os.path.join(ASSETS_DIR, "amber_report_bg.png")
    bg = Image.open(bg_path).convert("RGB")
    bg = ImageOps.fit(bg, (TARGET_WIDTH, TARGET_HEIGHT), Image.Resampling.LANCZOS)
    
    # Subtle dark overlay on the left to make text readable against the parchment
    overlay = Image.new('RGBA', bg.size, (0,0,0,0))
    draw_overlay = ImageDraw.Draw(overlay)
    for x in range(TARGET_WIDTH // 2):
        alpha = int(140 * (1 - x/(TARGET_WIDTH // 2)))
    ...
    # Paste reports onto bg - "Unfurled Scroll" layout (one slightly higher and angled)
    center_x = TARGET_WIDTH // 2 + 100
    
    # Rotate slightly for a natural look
    r1_styled_rot = r1_styled.rotate(3, expand=True, resample=Image.Resampling.BICUBIC)
    r2_styled_rot = r2_styled.rotate(-2, expand=True, resample=Image.Resampling.BICUBIC)

    bg.paste(r1_styled_rot, (center_x - r1_styled_rot.width + 120, (TARGET_HEIGHT - r1_styled_rot.height)//2 - 40), r1_styled_rot)
    bg.paste(r2_styled_rot, (center_x + 80, (TARGET_HEIGHT - r2_styled_rot.height)//2 + 60), r2_styled_rot)
    ...
    texts = {
        "TC": [("結緣贈送", 110), ("專屬八字五行分析報告", 60), ("沉穩安神 · 尋回內心", 45)],
        "SC": [("结缘赠送", 110), ("专属八字五行分析报告", 60), ("沉稳安神 · 寻回内心", 45)]
    }
    ...
    for lang, lines in texts.items():
        ...
        for text, size in lines:
            ...
            # Pale gold text with dark shadow for parchment background
            draw_text_with_shadow(draw, (120, y_offset), text, font, (255, 235, 205), (0,0,0,150))
            y_offset += size + 40

        out_dir = os.path.join(OUTPUT_BASE_DIR, lang)
        os.makedirs(out_dir, exist_ok=True)
        out_path = os.path.join(out_dir, "12_amber_report_promo.png")
        img_copy.convert("RGB").save(out_path, "PNG")
        print(f"Saved {out_path}")

def main():
    print("Processing Medicinal Amber images...")
    
    process_image("amber_cover.png", "00_amber_cover.png", 
                  marketing_texts_tc["amber_cover.png"], 
                  marketing_texts_sc["amber_cover.png"], 
                  is_front=True)
                  
    images_to_process = [
        "amber_01.png", "amber_02.png", "amber_03.png", "amber_04.png",
        "amber_05.png", "amber_06.png", "amber_07.png", "amber_08.png",
        "amber_09.png", "amber_10.png", "amber_11.png"
    ]
    
    for i, img_key in enumerate(images_to_process, 1):
        out_name = f"{i:02d}_amber.png"
