...
OUTPUT_BASE_DIR = "/Users/michaelng/Desktop/marketing/黄水晶白阿塞手串_ad_material_20260508"
...
# --- Marketing Texts ---
marketing_texts_tc = {
    "yellow_white_cover.png": ("黃水晶白阿塞手串", "招財聚氣·淨化心靈"),
    "yellow_white_01.png": ("溫婉大氣", "氣質非凡"),
    "yellow_white_02.png": ("舉手投足", "盡顯優雅"),
    "yellow_white_03.png": ("冰裂紋理", "獨特魅力"),
    "yellow_white_04.png": ("明暗交織", "質感非凡"),
    "yellow_white_05.png": ("純淨無暇", "宛如初雪"),
    "yellow_white_06.png": ("尊貴禮遇", "典藏佳品"),
    "yellow_white_07.png": ("絲滑觸感", "溫潤如玉"),
    "yellow_white_08.png": ("白瓷雅韻", "靜心凝神"),
    "yellow_white_09.png": ("光影流疊", "璀璨奪目"),
    "yellow_white_10.png": ("書香氣息", "傳承經典"),
    "yellow_white_11.png": ("晶瑩剔透", "純粹之美"),
    "yellow_white_12.png": ("自然原石", "能量匯聚"),
    "yellow_white_13.png": ("深邃神秘", "高雅格調"),
}

marketing_texts_sc = {
    "yellow_white_cover.png": ("黄水晶白阿塞手串", "招财聚气·净化心灵"),
    "yellow_white_01.png": ("温婉大气", "气质非凡"),
    "yellow_white_02.png": ("举手投足", "尽显优雅"),
    "yellow_white_03.png": ("冰裂纹理", "独特魅力"),
    "yellow_white_04.png": ("明暗交织", "质感非凡"),
    "yellow_white_05.png": ("纯净无暇", "宛如初雪"),
    "yellow_white_06.png": ("尊贵礼遇", "典藏佳品"),
    "yellow_white_07.png": ("丝滑触感", "温润如玉"),
    "yellow_white_08.png": ("白瓷雅韵", "静心凝神"),
    "yellow_white_09.png": ("光影流叠", "璀璨夺目"),
    "yellow_white_10.png": ("书香气息", "传承经典"),
    "yellow_white_11.png": ("晶莹剔透", "纯粹之美"),
    "yellow_white_12.png": ("自然原石", "能量汇聚"),
    "yellow_white_13.png": ("深邃神秘", "高雅格调"),
}
...
def add_border_and_shadow(img, border_size=5, shadow_size=35, shadow_opacity=110):
    # Soft warm gold/white border
    bordered = ImageOps.expand(img, border=border_size, fill='#fdf5e6') 
    shadow = Image.new('RGBA', (bordered.width + shadow_size*2, bordered.height + shadow_size*2), (0,0,0,0))
    shadow_draw = ImageDraw.Draw(shadow)
    # Warm golden brown shadow
    shadow_draw.rectangle([shadow_size, shadow_size, bordered.width+shadow_size, bordered.height+shadow_size], fill=(80,60,20,shadow_opacity))
...
def process_image(img_filename, out_filename, text_tc, text_sc, is_front=False):
    ...
    for lang, texts in [("TC", text_tc), ("SC", text_sc)]:
        ...
        # Pale gold/white text for dark backgrounds, deep warm brown for light
        text_fill = (255, 250, 240) if not is_light_bg else (60, 40, 20) 
...
def process_promo():
    print("Generating gifted report promo image (Golden Sunrise & Soft White Silk Style)...")
    
    # 1. Background
    bg_path = os.path.join(ASSETS_DIR, "yellow_white_report_bg.png")
    bg = Image.open(bg_path).convert("RGB")
    bg = ImageOps.fit(bg, (TARGET_WIDTH, TARGET_HEIGHT), Image.Resampling.LANCZOS)
    
    # Subtle dark overlay on the left for text readability against the bright background
    overlay = Image.new('RGBA', bg.size, (0,0,0,0))
    draw_overlay = ImageDraw.Draw(overlay)
    for x in range(TARGET_WIDTH // 2):
        alpha = int(120 * (1 - x/(TARGET_WIDTH // 2)))
        draw_overlay.line([(x, 0), (x, TARGET_HEIGHT)], fill=(40,20,10, alpha))
    ...
    center_x = TARGET_WIDTH // 2 + 180
    center_y = TARGET_HEIGHT // 2
    
    # Rotate slightly for a relaxed, dreamy feel
    r1_rot = r1_styled.rotate(3, expand=True, resample=Image.Resampling.BICUBIC)
    r2_rot = r2_styled.rotate(-2, expand=True, resample=Image.Resampling.BICUBIC)

    bg.paste(r1_rot, (center_x - r1_rot.width//2 - 60, center_y - r1_rot.height//2 + 40), r1_rot)
    bg.paste(r2_rot, (center_x - r2_rot.width//2 + 80, center_y - r2_rot.height//2 - 50), r2_rot)
    ...
    texts = {
        "TC": [("結緣贈送", 110), ("專屬八字五行分析報告", 60), ("招財聚氣 · 淨化心靈", 45)],
        "SC": [("结缘赠送", 110), ("专属八字五行分析报告", 60), ("招财聚气 · 净化心灵", 45)]
    }
    ...
    for lang, lines in texts.items():
        ...
        for text, size in lines:
            ...
            # Warm pale gold text with dark brown shadow
            draw_text_with_shadow(draw, (120, y_offset), text, font, (255, 245, 230), (60,30,10,220))
            y_offset += size + 40

        out_dir = os.path.join(OUTPUT_BASE_DIR, lang)
        os.makedirs(out_dir, exist_ok=True)
        out_path = os.path.join(out_dir, "14_yellow_white_report_promo.png")
        img_copy.convert("RGB").save(out_path, "PNG")
        print(f"Saved {out_path}")

def main():
    print("Processing Citrine & White Azeztulite images...")
    
    process_image("yellow_white_cover.png", "00_yellow_white_cover.png", 
                  marketing_texts_tc["yellow_white_cover.png"], 
                  marketing_texts_sc["yellow_white_cover.png"], 
                  is_front=True)
                  
    images_to_process = [
        "yellow_white_01.png", "yellow_white_02.png", "yellow_white_03.png", "yellow_white_04.png",
        "yellow_white_05.png", "yellow_white_06.png", "yellow_white_07.png", "yellow_white_08.png",
        "yellow_white_09.png", "yellow_white_10.png", "yellow_white_11.png", "yellow_white_12.png",
        "yellow_white_13.png"
    ]
    
    for i, img_key in enumerate(images_to_process, 1):
        out_name = f"{i:02d}_yellow_white.png"
