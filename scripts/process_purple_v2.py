...
OUTPUT_BASE_DIR = "/Users/michaelng/Desktop/marketing/无加色天然紫幽灵手链_ad_material_20260507"
...
# --- Marketing Texts ---
marketing_texts_tc = {
    "purple_v2_cover.png": ("無加色天然紫幽靈", "招財納福·提升氣質"),
    "purple_v2_01.png": ("溫潤如玉", "氣質優雅"),
    "purple_v2_02.png": ("純淨天然", "無加色處理"),
    "purple_v2_03.png": ("異象景觀", "獨一無二"),
    "purple_v2_04.png": ("靜心品茗", "禪意生活"),
    "purple_v2_05.png": ("完美好禮", "傳遞心意"),
    "purple_v2_06.png": ("職場穿搭", "自信從容"),
    "purple_v2_07.png": ("寧靜致遠", "平衡身心"),
    "purple_v2_08.png": ("書香氣息", "溫暖陪伴"),
    "purple_v2_09.png": ("時尚百搭", "點亮細節"),
    "purple_v2_10.png": ("陽光折射", "耀眼光芒"),
    "purple_v2_11.png": ("晶瑩剔透", "質感非凡"),
}

marketing_texts_sc = {
    "purple_v2_cover.png": ("无加色天然紫幽灵", "招财纳福·提升气质"),
    "purple_v2_01.png": ("温润如玉", "气质优雅"),
    "purple_v2_02.png": ("纯净天然", "无加色处理"),
    "purple_v2_03.png": ("异象景观", "独一无二"),
    "purple_v2_04.png": ("静心品茗", "禅意生活"),
    "purple_v2_05.png": ("完美好礼", "传递心意"),
    "purple_v2_06.png": ("职场穿搭", "自信从容"),
    "purple_v2_07.png": ("宁静致远", "平衡身心"),
    "purple_v2_08.png": ("书香气息", "温暖陪伴"),
    "purple_v2_09.png": ("时尚百搭", "点亮细节"),
    "purple_v2_10.png": ("阳光折射", "耀眼光芒"),
    "purple_v2_11.png": ("晶莹剔透", "质感非凡"),
}
...
def add_border_and_shadow(img, border_size=2, shadow_size=20):
    bordered = ImageOps.expand(img, border=border_size, fill='#f5f5dc') # warm beige border
    shadow = Image.new('RGBA', (bordered.width + shadow_size*2, bordered.height + shadow_size*2), (0,0,0,0))
    shadow_draw = ImageDraw.Draw(shadow)
    shadow_draw.rectangle([shadow_size, shadow_size, bordered.width+shadow_size, bordered.height+shadow_size], fill=(0,0,0,50))
...
def process_promo():
    print("Generating gifted report promo image (Zen Tea Style)...")
    
    # 1. Background - Zen Tea Table Style
    bg_path = os.path.join(ASSETS_DIR, "purple_v2_report_bg.png")
    bg = Image.open(bg_path).convert("RGB")
    bg = ImageOps.fit(bg, (TARGET_WIDTH, TARGET_HEIGHT), Image.Resampling.LANCZOS)
    
    # Subtle dark overlay to make text and reports readable
    overlay = Image.new('RGBA', bg.size, (0,0,0,0))
    draw_overlay = ImageDraw.Draw(overlay)
    for x in range(TARGET_WIDTH // 2):
        alpha = int(160 * (1 - x/(TARGET_WIDTH // 2)))
    ...
    texts = {
        "TC": [("結緣贈送", 110), ("專屬八字五行分析報告", 60), ("禪意生活 · 順遂如意", 45)],
        "SC": [("结缘赠送", 110), ("专属八字五行分析报告", 60), ("禅意生活 · 顺遂如意", 45)]
    }
    ...
    for lang, lines in texts.items():
        ...
        out_dir = os.path.join(OUTPUT_BASE_DIR, lang)
        os.makedirs(out_dir, exist_ok=True)
        out_path = os.path.join(out_dir, "12_purple_v2_report_promo.png")
        img_copy.convert("RGB").save(out_path, "PNG")
        print(f"Saved {out_path}")

def main():
    print("Processing Uncolored Natural Purple Phantom Quartz images...")
    
    process_image("purple_v2_cover.png", "00_purple_v2_cover.png", 
                  marketing_texts_tc["purple_v2_cover.png"], 
                  marketing_texts_sc["purple_v2_cover.png"], 
                  is_front=True)
                  
    images_to_process = [
        "purple_v2_01.png", "purple_v2_02.png", "purple_v2_03.png", "purple_v2_04.png",
        "purple_v2_05.png", "purple_v2_06.png", "purple_v2_07.png", "purple_v2_08.png",
        "purple_v2_09.png", "purple_v2_10.png", "purple_v2_11.png"
    ]
    
    for i, img_key in enumerate(images_to_process, 1):
        out_name = f"{i:02d}_purple_v2.png"
