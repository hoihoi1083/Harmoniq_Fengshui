...
OUTPUT_BASE_DIR = "/Users/michaelng/Desktop/marketing/血珀手串_ad_material_20260507"
...
# --- Marketing Texts ---
marketing_texts_tc = {
    "blood_amber_cover.png": ("天然血珀手串", "明艷動人·氣血紅潤"),
    "blood_amber_01.png": ("酒紅晶瑩", "高貴典雅"),
    "blood_amber_02.png": ("光澤流轉", "熱情如火"),
    "blood_amber_03.png": ("浪漫氣息", "遇見美好"),
    "blood_amber_04.png": ("奢華質感", "品味非凡"),
    "blood_amber_05.png": ("深邃迷人", "彰顯魅力"),
    "blood_amber_06.png": ("日常穿搭", "點亮細節"),
    "blood_amber_07.png": ("陽光折射", "閃耀動人"),
    "blood_amber_08.png": ("靜享時光", "溫暖陪伴"),
    "blood_amber_09.png": ("時尚百搭", "精緻優雅"),
    "blood_amber_10.png": ("質感細膩", "觸手生溫"),
    "blood_amber_11.png": ("護身祈福", "平安喜樂"),
}

marketing_texts_sc = {
    "blood_amber_cover.png": ("天然血珀手串", "明艳动人·气血红润"),
    "blood_amber_01.png": ("酒红晶莹", "高贵典雅"),
    "blood_amber_02.png": ("光泽流转", "热情如火"),
    "blood_amber_03.png": ("浪漫气息", "遇见美好"),
    "blood_amber_04.png": ("奢华质感", "品味非凡"),
    "blood_amber_05.png": ("深邃迷人", "彰显魅力"),
    "blood_amber_06.png": ("日常穿搭", "点亮细节"),
    "blood_amber_07.png": ("阳光折射", "闪耀动人"),
    "blood_amber_08.png": ("静享时光", "温暖陪伴"),
    "blood_amber_09.png": ("时尚百搭", "精致优雅"),
    "blood_amber_10.png": ("质感细腻", "触手生温"),
    "blood_amber_11.png": ("护身祈福", "平安喜乐"),
}
...
def add_border_and_shadow(img, border_size=4, shadow_size=30, shadow_opacity=100):
    # Use a dark, rich burgundy border
    bordered = ImageOps.expand(img, border=border_size, fill='#4a0404') 
...
def process_promo():
    print("Generating gifted report promo image (Burgundy Velvet Style)...")
    
    # 1. Background - Burgundy Velvet Style
    bg_path = os.path.join(ASSETS_DIR, "blood_amber_report_bg.png")
    bg = Image.open(bg_path).convert("RGB")
    bg = ImageOps.fit(bg, (TARGET_WIDTH, TARGET_HEIGHT), Image.Resampling.LANCZOS)
    
    # Subtle dark overlay on the left to make text readable against the rich velvet
    ...
    # Rotate slightly for a dynamic look
    r1_styled_rot = r1_styled.rotate(-4, expand=True, resample=Image.Resampling.BICUBIC)
    r2_styled_rot = r2_styled.rotate(2, expand=True, resample=Image.Resampling.BICUBIC)

    bg.paste(r1_styled_rot, (center_x - r1_styled_rot.width + 150, (TARGET_HEIGHT - r1_styled_rot.height)//2 - 60), r1_styled_rot)
    bg.paste(r2_styled_rot, (center_x + 50, (TARGET_HEIGHT - r2_styled_rot.height)//2 + 40), r2_styled_rot)
    ...
    texts = {
        "TC": [("結緣贈送", 110), ("專屬八字五行分析報告", 60), ("明艷動人 · 氣血紅潤", 45)],
        "SC": [("结缘赠送", 110), ("专属八字五行分析报告", 60), ("明艳动人 · 气血红润", 45)]
    }
    ...
    for lang, lines in texts.items():
        ...
        for text, size in lines:
            ...
            # Pale gold text with dark shadow for velvet background
            draw_text_with_shadow(draw, (120, y_offset), text, font, (255, 235, 205), (0,0,0,180))
            y_offset += size + 40

        out_dir = os.path.join(OUTPUT_BASE_DIR, lang)
        os.makedirs(out_dir, exist_ok=True)
        out_path = os.path.join(out_dir, "12_blood_amber_report_promo.png")
        img_copy.convert("RGB").save(out_path, "PNG")
        print(f"Saved {out_path}")

def main():
    print("Processing Blood Amber images...")
    
    process_image("blood_amber_cover.png", "00_blood_amber_cover.png", 
                  marketing_texts_tc["blood_amber_cover.png"], 
                  marketing_texts_sc["blood_amber_cover.png"], 
                  is_front=True)
                  
    images_to_process = [
        "blood_amber_01.png", "blood_amber_02.png", "blood_amber_03.png", "blood_amber_04.png",
        "blood_amber_05.png", "blood_amber_06.png", "blood_amber_07.png", "blood_amber_08.png",
        "blood_amber_09.png", "blood_amber_10.png", "blood_amber_11.png"
    ]
    
    for i, img_key in enumerate(images_to_process, 1):
        out_name = f"{i:02d}_blood_amber.png"
