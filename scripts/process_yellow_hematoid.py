...
OUTPUT_BASE_DIR = "/Users/michaelng/Desktop/marketing/黄胶花手串_ad_material_20260507"
...
# --- Marketing Texts ---
marketing_texts_tc = {
    "yellow_cover.png": ("黃膠花手串", "招財納福·事業高升"),
    "yellow_01.png": ("溫潤如玉", "氣質優雅"),
    "yellow_02.png": ("絲滑觸感", "尊貴享受"),
    "yellow_03.png": ("靜心品茗", "禪意生活"),
    "yellow_04.png": ("完美好禮", "傳遞心意"),
    "yellow_05.png": ("自然之美", "靈動生機"),
    "yellow_06.png": ("職場穿搭", "自信從容"),
    "yellow_07.png": ("異象景觀", "獨一無二"),
    "yellow_08.png": ("寧靜致遠", "平衡身心"),
    "yellow_09.png": ("悠閒時光", "溫暖陪伴"),
    "yellow_10.png": ("時尚百搭", "點亮細節"),
    "yellow_11.png": ("陽光折射", "耀眼光芒"),
}

marketing_texts_sc = {
    "yellow_cover.png": ("黄胶花手串", "招财纳福·事业高升"),
    "yellow_01.png": ("温润如玉", "气质优雅"),
    "yellow_02.png": ("丝滑触感", "尊贵享受"),
    "yellow_03.png": ("静心品茗", "禅意生活"),
    "yellow_04.png": ("完美好礼", "传递心意"),
    "yellow_05.png": ("自然之美", "灵动生机"),
    "yellow_06.png": ("职场穿搭", "自信从容"),
    "yellow_07.png": ("异象景观", "独一无二"),
    "yellow_08.png": ("宁静致远", "平衡身心"),
    "yellow_09.png": ("悠闲时光", "温暖陪伴"),
    "yellow_10.png": ("时尚百搭", "点亮细节"),
    "yellow_11.png": ("阳光折射", "耀眼光芒"),
}
...
def add_border_and_shadow(img, border_size=2, shadow_size=20):
    bordered = ImageOps.expand(img, border=border_size, fill='#e0e0e0') # subtle grey border
    ...
    shadow_draw.rectangle([shadow_size, shadow_size, bordered.width+shadow_size, bordered.height+shadow_size], fill=(0,0,0,40))
...
def process_promo():
    print("Generating gifted report promo image (New Style)...")
    
    # 1. Background - New Marble Style
    bg_path = os.path.join(ASSETS_DIR, "yellow_report_bg.png")
    bg = Image.open(bg_path).convert("RGB")
    bg = ImageOps.fit(bg, (TARGET_WIDTH, TARGET_HEIGHT), Image.Resampling.LANCZOS)
    
    # No dark overlay, keep it bright and clean. Just a very subtle white gradient on the left for text.
    ...
    for lang, lines in texts.items():
        ...
        for text, size in lines:
            ...
            # Dark charcoal text with white shadow for the bright marble background
            draw_text_with_shadow(draw, (120, y_offset), text, font, (40, 40, 40), (255,255,255,200))
            y_offset += size + 40

        out_dir = os.path.join(OUTPUT_BASE_DIR, lang)
        os.makedirs(out_dir, exist_ok=True)
        out_path = os.path.join(out_dir, "12_yellow_hematoid_report_promo.png")
        img_copy.convert("RGB").save(out_path, "PNG")
        print(f"Saved {out_path}")

def main():
    print("Processing Yellow Hematoid Quartz images...")
    
    process_image("yellow_cover.png", "00_yellow_hematoid_cover.png", 
                  marketing_texts_tc["yellow_cover.png"], 
                  marketing_texts_sc["yellow_cover.png"], 
                  is_front=True)
                  
    images_to_process = [
        "yellow_01.png", "yellow_02.png", "yellow_03.png", "yellow_04.png",
        "yellow_05.png", "yellow_06.png", "yellow_07.png", "yellow_08.png",
        "yellow_09.png", "yellow_10.png", "yellow_11.png"
    ]
    
    for i, img_key in enumerate(images_to_process, 1):
        out_name = f"{i:02d}_yellow_hematoid.png"
