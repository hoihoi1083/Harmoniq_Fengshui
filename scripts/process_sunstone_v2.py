...
OUTPUT_BASE_DIR = "/Users/michaelng/Desktop/marketing/太阳石手串_ad_material_20260507"
...
# --- Marketing Texts ---
marketing_texts_tc = {
    "sunstone_v2_cover.png": ("天然太陽石手串", "溫暖閃耀·正能量"),
    "sunstone_v2_01.png": ("溫潤如玉", "光芒內斂"),
    "sunstone_v2_02.png": ("貓眼效應", "靈動閃爍"),
    "sunstone_v2_03.png": ("天然礦缺", "獨特印記"),
    "sunstone_v2_04.png": ("氣質百搭", "點亮腕間"),
    "sunstone_v2_05.png": ("柔和蜜桃色", "甜美溫婉"),
    "sunstone_v2_06.png": ("日常穿搭", "彰顯品味"),
    "sunstone_v2_07.png": ("陽光折射", "熠熠生輝"),
    "sunstone_v2_08.png": ("靜享時光", "溫暖陪伴"),
    "sunstone_v2_09.png": ("細膩質感", "觸手生溫"),
    "sunstone_v2_10.png": ("浪漫點綴", "遇見美好"),
    "sunstone_v2_11.png": ("護身祈福", "驅散陰霾"),
}

marketing_texts_sc = {
    "sunstone_v2_cover.png": ("天然太阳石手串", "温暖闪耀·正能量"),
    "sunstone_v2_01.png": ("温润如玉", "光芒内敛"),
    "sunstone_v2_02.png": ("猫眼效应", "灵动闪烁"),
    "sunstone_v2_03.png": ("天然矿缺", "独特印记"),
    "sunstone_v2_04.png": ("气质百搭", "点亮腕间"),
    "sunstone_v2_05.png": ("柔和蜜桃色", "甜美温婉"),
    "sunstone_v2_06.png": ("日常穿搭", "彰显品味"),
    "sunstone_v2_07.png": ("阳光折射", "熠熠生辉"),
    "sunstone_v2_08.png": ("静享时光", "温暖陪伴"),
    "sunstone_v2_09.png": ("细腻质感", "触手生温"),
    "sunstone_v2_10.png": ("浪漫点缀", "遇见美好"),
    "sunstone_v2_11.png": ("护身祈福", "驱散阴霾"),
}
...
def add_border_and_shadow(img, border_size=3, shadow_size=25, shadow_opacity=60):
    # Use a soft, warm peach/rose gold border
    bordered = ImageOps.expand(img, border=border_size, fill='#ffdab9') 
...
def process_promo():
    print("Generating gifted report promo image (Soft Peach Silk Style)...")
    
    # 1. Background - Soft Peach Silk Style
    bg_path = os.path.join(ASSETS_DIR, "sunstone_v2_report_bg.png")
    bg = Image.open(bg_path).convert("RGB")
    bg = ImageOps.fit(bg, (TARGET_WIDTH, TARGET_HEIGHT), Image.Resampling.LANCZOS)
    
    # Subtle white overlay on the left to make text readable against the bright silk
    ...
    # Rotate slightly for a flowing look
    r1_styled_rot = r1_styled.rotate(-2, expand=True, resample=Image.Resampling.BICUBIC)
    r2_styled_rot = r2_styled.rotate(1, expand=True, resample=Image.Resampling.BICUBIC)

    bg.paste(r1_styled_rot, (center_x - r1_styled_rot.width + 80, (TARGET_HEIGHT - r1_styled_rot.height)//2 - 60), r1_styled_rot)
    bg.paste(r2_styled_rot, (center_x + 30, (TARGET_HEIGHT - r2_styled_rot.height)//2 + 60), r2_styled_rot)
    ...
    texts = {
        "TC": [("結緣贈送", 110), ("專屬八字五行分析報告", 60), ("溫暖閃耀 · 迎接好運", 45)],
        "SC": [("结缘赠送", 110), ("专属八字五行分析报告", 60), ("温暖闪耀 · 迎接好运", 45)]
    }
    ...
    for lang, lines in texts.items():
        ...
        for text, size in lines:
            ...
            # Dark charcoal/brown text with white shadow for bright peach background
            draw_text_with_shadow(draw, (120, y_offset), text, font, (70, 50, 40), (255,255,255,180))
            y_offset += size + 40

        out_dir = os.path.join(OUTPUT_BASE_DIR, lang)
        os.makedirs(out_dir, exist_ok=True)
        out_path = os.path.join(out_dir, "12_sunstone_v2_report_promo.png")
        img_copy.convert("RGB").save(out_path, "PNG")
        print(f"Saved {out_path}")

def main():
    print("Processing Sunstone images...")
    
    process_image("sunstone_v2_cover.png", "00_sunstone_cover.png", 
                  marketing_texts_tc["sunstone_v2_cover.png"], 
                  marketing_texts_sc["sunstone_v2_cover.png"], 
                  is_front=True)
                  
    images_to_process = [
        "sunstone_v2_01.png", "sunstone_v2_02.png", "sunstone_v2_03.png", "sunstone_v2_04.png",
        "sunstone_v2_05.png", "sunstone_v2_06.png", "sunstone_v2_07.png", "sunstone_v2_08.png",
        "sunstone_v2_09.png", "sunstone_v2_10.png", "sunstone_v2_11.png"
    ]
